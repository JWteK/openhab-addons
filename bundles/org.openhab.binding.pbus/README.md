...
The Pbus binding integrates with Siemens PTM modules connected to a PTMod via an USB-RS422/RS485 module (based on a FTDI chipset) or a network connection (TCP/IP).

The binding exposes basic actions from the Pbus System that can be triggered from the smartphone/tablet interface, as defined by the [Velbus Protocol info sheets](https://github.com/Velbus).
...
`bundles/org.openhab.binding.pbus/src/main/resources/OH-INF/thing/thing-types.xml` file. The
...
The Pbus bridge cannot be discovered automatically.
Configure it manually by defining the serial port of the Pbus Configuration module for the Pbus Serial Bridge or by defining the IP address and port for the Pbus Network Bridge, as described in the [`Thing Configuration`](#thing-configuration) section.

Once the bridge has been configured with openHAB, a manual scan can be initiated to discover Pbus modules with an assigned address.
Addresses can be assigned via the official configuration software, and is a required step before a Pbus installation can work correctly.

The discovery scan can take a few minutes to complete.
Modules discovered during this scan will appear in the inbox. This procedure will also retrieve the channel names of the Pbus devices.

## Thing Configuration

The Pbus bridge needs to be added first.

### Pbus Serial Bridge

For the Pbus Serial Bridge it is necessary to specify the serial port device used for communication.

On Linux and other UNIX systems, it is recommended to use a more stable symbolic device path such as `/dev/serial/by-id/usb-Velleman_Projects_VMB1USB_Pbus_USB_interface-if00`, as it will always refer at to a Pbus configuration module, and not an arbitrary serial device.
...
In a `.things` file, a USB connection to a Pbus configuration module might be configured like so:

```java
Bridge Pbus:bridge:1 [ port="/dev/serial/by-id/usb-Velleman_Projects_VMB1USB_Pbus_USB_interface-if00" ]
// or
Bridge Pbus:bridge:1 [ port="COM1" ]
```

### Pbus Network Bridge

For the Pbus Network Bridge it is necessary to specify the address (either an IP address or a hostname) and the port of a Pbus network server.

In a `.things` file, a network bridge running on the same machine at port 6000 would be configured like so:

```java
Bridge Pbus:networkbridge:1 "Pbus Network Bridge - Loopback" @ "Control" [ address="localhost", port=6000 ]
```

### Realtime Clock Synchronization

Optionally, the openHAB Pbus binding can synchronize the realtime clock, date and daylight savings status of the Pbus modules.
This is achieved by setting the Time Update Interval (in minutes) on the bridge thing. For example:

```java
Bridge Pbus:bridge:1 [ port="COM1", timeUpdateInterval="360" ]
```
...
In case of a connection error, a Pbus bridge will attempt to reconnect every 15 seconds by default.
You can modify the bridge reconnection interval by specifying the `reconnectionInterval` parameter (in seconds):

```java
Bridge Pbus:bridge:1 [ port="COM1", reconnectionInterval="15" ]
```

### Pbus modules

Adding Pbus modules to your openHAB configuration follows the conventions of your preferred configuration method.
...
* **Textual `.thing` configuration** can declare Pbus modules either in a standalone fashion (a bridge is still required):

  ```java
  Thing Pbus:<thingType>:<bridgeId>:<thingId> "Label" @ "Location" [ CH1="Kitchen Light", CH2="Living Light" ]
  ```
  ...
  * `<thingType>` is the type of the Pbus module. Refer to the [Supported Things](#supported-things) table for valid `<thingType>` values;
  * `<thingId>` is the hexadecimal address of the Pbus module;
...
Individual module `Thing`’s channels can be linked to openHAB items via channel names like `Pbus:vmb4ryld:1:0A:CH1`. Here, from left to right, the channel name consistes of the binding name, module type (`<thingType> = vmb4ryld`), bridge id (`1`), module’s hexadecimal address (`<thingId> = 0A`) and channel within the module (`CH1`).
...
`bundles/org.openhab.binding.Pbus/src/main/resources/OH-INF/config/thing-config.xml` file.
...
xidel -e 'fn:for-each(//parameter, function($p) { "| `" || $p/@name || "` | " || $p/description || " |" })' \
    src/main/resources/OH-INF/config/thing-config.xml \
| sort -u \
...
```java
Switch LivingRoom           { channel="Pbus:vmb4ryld:1:06:CH1" }                # Switch for onOff type action
Switch KitchenButton        { channel="Pbus:vmb2pbn:1:05:button#CH1" }          # Switch for Pressed and Long_Pressed type actions
Dimmer TVRoom               { channel="Pbus:vmb4dc:1:07:CH2" }                  # Changing brightness dimmer type action
Rollershutter Kitchen       { channel="Pbus:vmb2ble:1:01" }                     # Controlling rollershutter or blind type action

Number Temperature_LivingRoom "Temperature [%.1f °C]"     <temperature> { channel="Pbus:vmbgp1:1:08:CH09" }
Number Temperature_Corridor   "Temperature [%.1f °C]"     <temperature> { channel="Pbus:vmbgpo:1:0C:CH33" }
Number Temperature_Outside    "Temperature [%.1f °C]"     <temperature> { channel="Pbus:vmbpiro:1:0E:CH09" }
```
...
when
    Channel 'Pbus:vmb7in:1:05:CH5' triggered PRESSED
then
    var message = receivedEvent.getEvent()
    logInfo("PbusTriggerExample", "Message: {}", message)
    ...
end
```
